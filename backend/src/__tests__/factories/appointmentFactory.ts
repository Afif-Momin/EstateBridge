...
export const createAppointmentDTOArbitrary = fc.record({
  listingId: fc.uuid(),
  sellerId: fc.uuid(),
  requestedDateTime: fc.date({ min: new Date() }),
  reason_to_buy: fc.constantFrom('Investment' as const, 'Self Use' as const),
...
export class AppointmentFactory {
  static create(overrides?: Partial<Appointment>): Appointment {
    ...
    return {
      id: `appointment-${Date.now()}-${Math.random().toString(36).substring(7)}`,
  ...
  static createDTO(overrides?: Partial<CreateAppointmentDTO>): CreateAppointmentDTO {
